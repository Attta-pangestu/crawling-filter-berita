...
from convert_date_helper import convert_date_format
from keyword_counter_helper import keyword_counter_filter
...
def get_link_info(result):
  ...
  date_elements = result.find_elements(By.CSS_SELECTOR, '.date')
  try:
    if date_elements:
      # Ambil Link
      # Ambil Link
...
def search_with_pagination(search_keyword, start_date, end_date, num_pages=10):
  ...
  try:
    ...
    # Sisipkan skrip JavadriverScript untuk menghentikan loading
